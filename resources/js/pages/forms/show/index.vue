        ...
        <div class="w-full md:w-4/5 lg:w-3/5 md:mx-auto md:max-w-4xl px-4">
          <div class="pt-4 pb-0">
            ...
            <ul class="flex text-gray-500">
              ...
              <li class="list-disc ml-6 pr-1 text-blue-500" v-if="form.visibility=='draft'">Draft</li>
              <li class="list-disc ml-6 pr-1 text-blue-500" v-if="form.visibility=='closed'">Fechada</li>
              <li class="list-disc ml-6">Editada {{ form.last_edited_human }}</li>
            </ul>

            <p v-if="form.closes_at" class="text-yellow-500">
              <span v-if="form.is_closed"> Esta form parou de receber submissões a {{
                  displayClosesDate
                }} </span>
              <span v-else> Esta form vai parar de receber submissões a {{ displayClosesDate }} </span>
            </p>
            <p v-if="form.max_submissions_count > 0" class="text-yellow-500">
              <span v-if="form.max_number_of_submissions_reached"> Esta form atingiu o limite máximo de submissões: {{
                  form.max_submissions_count
                }} submissões. </span>
              <span v-else> Limite máximo de submissões: {{ form.max_submissions_count }}  </span>
            </p>
